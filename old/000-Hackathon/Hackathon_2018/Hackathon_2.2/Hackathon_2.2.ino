...
void setup() { // Einmalige Initialisierung
  ...
  delay(100);
  Serial.print ("\nWLAN connect to:");
  Serial.print("weihdorfer3"); //------------------------------------------------------------------------------------------------------------------------------------
  WiFi.begin("weihdorfer3", "schneewittchen");
  for (int i; i < 20 ; i++) { // Warte bis Verbindung steht(max 10sek)
    delay(500);
    Serial.print(".");
  ...
  if (WlanVerbindung = 1) {
    matrixausgabe_text = "Link";
    matrixausgabe_index = 0;
  } else {
    matrixausgabe_text = "no_Link";
    matrixausgabe_index = 0;
  }
...
void co2()
{
  ...
  {
    if (( ( ( co2_eq_ppm ) < ( 1500 ) ) && ( ( co2_eq_ppm ) > ( 1000 ) ) ))
    {
